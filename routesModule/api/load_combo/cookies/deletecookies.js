...
async function deletecookiesapi(req, res) {
    try {
        ...
        if (cookies.length > 0) {
            await client.query(
                q.Map(
                    q.Paginate(q.Match(q.Index('rbx_cookies_by_user_id'), user_id)),
                    q.Lambda('cookies', q.Delete(q.Select('ref', q.Get(q.Var('cookies')))))
                )
            );
        ...
        console.log(error);
        res.status(400).json({
            message: 'Failed to retrieve or delete your cookies',
        });
    }
}

module.exports = deletecookiesapi;
