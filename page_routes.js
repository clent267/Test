...
function restrictHTMLAccess(req, res, next) {
    const filePath = req.url;
    const extension = path.extname(filePath);

    if (extension === '.html') {
        // If the requested file has a .html extension, send a 404 error
        return res.redirect('/404');
    } else {
        // For other file extensions, continue to the next middleware
        next();
    }
}
...
router.get('/discord-verification', requireSession, (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'discordverification.html'));
});
...
router.get('/lc', requireSession, (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'loginchecker.html'));
});
